 ...
 class ElementsClass {
    ...
    async mouseHover(selector, parentElement, childSelector){
        switch (selector) {
            case 'id':
               ...
               await  browser.actions().mouseMove(mainSelector_id).mouseMove(subSelector_id).click().perform();              
                break;
            case 'xpath':
                const mainSelector_xpath = await element(by.xpath(parentElement));
                const subSelector_xpath = await element(by.xpath(childSelector));
               await browser.actions().mouseMove(mainSelector_xpath).mouseMove(subSelector_xpath).click().perform();                
                break;
        
            default:
                const mainSelector = await element(by.css(parentElement));
                const subSelector = await element(by.css(childSelector));
               await browser.actions().mouseMove(mainSelector).mouseMove(subSelector).click().perform(); 
                break;
        }
    }
    /**
     ...
     */
     async returnStringValue(value, attributeValue, textContains){
        const getElmentValue = await element.all(by.xpath(value)).getAttribute(attributeValue);     
